...
{
    public partial class userControlRenterAdd : UserControl
    {
        
        string type1, availableFor1, location1, rent1, phonenumber1, details1;

        addDeleteService deleteService=new addDeleteService();

        int id1;
        Image pic1, pic2;
        public userControlRenterAdd(string type, string availableFor, string location, string rent, int id, Image img1, Image img2, string phonenumber, string details)
        {
            InitializeComponent();
            ltype.Text = type;
        ...
        {
            deleteService.deleteAddData(id1);
            this.Hide();
        }
    }
}
